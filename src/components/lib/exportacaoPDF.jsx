...
const formatOrcamentoPrintStatus = (status) => {
...
export function gerarPDFOrcamento(orcamento, { empresa = {}, clienteNome = '', condicaoPagamento = '' } = {}) {
  ...
  const status = formatOrcamentoPrintStatus(orcamento.status);
  const versao = orcamento.versao != null ? String(orcamento.versao) : '1';
  const origem = orcamento.origem || 'MANUAL';
  const canal = orcamento.canal || '-';
  const tituloNumero = orcamento.numero
    ? `Orçamento ${escapeDocumentText(orcamento.numero)} · v${escapeDocumentText(versao)}`
    : 'Orçamento';
  const html = `<!doctype html><html><head><meta charset="UTF-8"><title>${tituloNumero}</title><style>
    ...
    <header class="header"><div><div class="brand">${escapeDocumentText(empresaNome)}</div><div class="muted">${escapeDocumentText(documentoEmpresa)}</div></div><div class="right"><strong>${tituloNumero}</strong><br>${escapeDocumentText(status)}<br><span class="muted">Origem ${escapeDocumentText(origem)} · Canal ${escapeDocumentText(canal)}</span></div></header>
    <div class="title">PROPOSTA COMERCIAL</div><section class="grid"><div class="box"><strong>Cliente</strong><br>${escapeDocumentText(clienteNome)}</div><div class="box"><strong>Condição de pagamento</strong><br>${escapeDocumentText(condicaoPagamento)}<br><strong>Validade</strong>: ${escapeDocumentText(orcamento.validade_em ? new Date(orcamento.validade_em).toLocaleDateString('pt-BR') : '-')}</div></section>
    ...
    <div class="notes"><strong>Observações</strong><br>${escapeDocumentText(orcamento.observacoes || 'Sem observações.')}</div><footer class="footer">Documento gerado em ${escapeDocumentText(new Date().toLocaleString('pt-BR'))} pelo ERP Zuccaro · empresa emissora exclusiva</footer></body></html>`;
