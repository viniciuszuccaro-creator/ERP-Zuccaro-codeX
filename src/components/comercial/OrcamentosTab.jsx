...
import { buildOrcamentoPayload, buildOrcamentoShareText, buildOrcamentoVersionPayload, calculateItem, calculateTotals, canUseOrcamentoAction, formatOrcamentoStatusLabel, microsToDecimal, ORCAMENTO_ORIGEM_LABELS, prepareOrcamentoAnexoFile } from './orcamentoUiPolicy';
...
export default function OrcamentosTab({ groupId, empresaId, actorId, actorEmail, empresaAtual, hasPermission, filterInContext, createInContext, windowMode = false }) {
  ...
  const registerAnexo = async (event) => {
    const file = event.target.files?.[0];
    if (!file || !selected || !canEdit(selected) || submitting) return;
    setSubmitting(true);
    try {
      const meta = prepareOrcamentoAnexoFile(file, { groupId, empresaId, orcamentoId: selected.id });
      const digest = await crypto.subtle.digest('SHA-256', await file.arrayBuffer());
      const sha256 = [...new Uint8Array(digest)].map((b) => b.toString(16).padStart(2, '0')).join('');
      const created = await api.registerAnexo(selected.id, { ...meta, sha256 });
      setAnexos(await api.listAnexos(selected.id).catch(() => []));
      toast.success(`Anexo ${created?.nome_arquivo || meta.nome_arquivo} registrado.`);
    } catch (error) { toast.error(error?.message || errorMessage(error)); }
    finally {
      setSubmitting(false);
      if (event.target) event.target.value = '';
    }
  };
  const inactivateAnexo = async (anexo) => {
    if (!selected || !anexo || !canEdit(selected) || submitting) return;
    if (!window.confirm(`Inativar o anexo ${anexo.nome_arquivo}?`)) return;
    setSubmitting(true);
    try {
      await api.deactivateAnexo(selected.id, anexo.id);
      setAnexos(await api.listAnexos(selected.id).catch(() => []));
      toast.success('Anexo inativado.');
    } catch (error) { toast.error(errorMessage(error)); }
    finally { setSubmitting(false); }
  };
