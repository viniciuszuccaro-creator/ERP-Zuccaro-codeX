...
const ctx: RequestContext = {
  groupId, empresaId, actorId, requestId: 'req-onda4-anexo', actorEmail: 'synthetic@example.invalid',
};
...
const quotePayload = {
  ...
  validade_em: '2027-08-01T00:00:00.000Z',
  itens: [{
    produto_id: produtoId,
    unidade_id: unidadeId,
    descricao: 'Item anexo',
    unidade_sigla: 'UN',
    quantidade: '1',
    preco_unitario: '10',
    desconto: '0',
...
test('migration 029 cria orcamento_anexos no agregado', async () => {
  const source = await readFile(new URL('../migrations/029_orcamentos_anexos.sql', import.meta.url), 'utf8');
  for (const required of ['orcamento_anexos', 'storage_key', 'QUARENTENA', 'idx_orcamento_anexos_scope']) {
    assert.ok(source.includes(required), required);
  }
});

test('StoragePort aceita entity Orcamento em path /orcamentos/', async () => {
  ...
  const orcamentoId = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb';
  const storageKey = `groups/${groupId}/companies/${empresaId}/orcamentos/${orcamentoId}/documents/${fileId}-proposta.pdf`;
  await assert.rejects(
    () => adapter.createSignedUploadUrl({
      groupId, empresaId, actorId, entity: 'Orcamento', entityId: orcamentoId,
      storageKey, fileName: 'proposta.pdf', mimeType: 'application/pdf',
      sizeBytes: 12, sha256: 'a'.repeat(64),
    }),
    // fetch fake pode falhar depois do assertContext — escopo inválido NÃO deve ocorrer
    (error: Error) => !/STORAGE_SCOPE_INVALID/.test(String(error.message)),
  );
  await assert.rejects(
    () => adapter.createSignedUploadUrl({
      groupId, empresaId, actorId, entity: 'Orcamento', entityId: orcamentoId,
      storageKey: storageKey.replace('/orcamentos/', '/products/'), fileName: 'proposta.pdf',
...
test('registerAnexo path fail-closed; lista e inativa', async () => {
  const { service, audit } = fixture();
  const quote = await service.create(ctx, quotePayload);
  const storageKey = `groups/${groupId}/companies/${empresaId}/orcamentos/${quote.id}/documents/${fileId}-desenho.pdf`;

  await assert.rejects(
    () => service.registerAnexo(ctx, quote.id, {
      storage_key: `groups/${groupId}/companies/${empresaId}/products/${produtoId}/documents/${fileId}-x.pdf`,
      nome_arquivo: 'x.pdf',
      mime_type: 'application/pdf',
      tamanho_bytes: 10,
      sha256: 'b'.repeat(64),
    }),
    (error: { code?: string }) => error.code === 'ORCAMENTO_ANEXO_PATH_INVALID',
  );

  const created = await service.registerAnexo(ctx, quote.id, {
  ...
  assert.equal(created.orcamento_id, quote.id);

  const listed = await service.listAnexos(ctx, quote.id);
  assert.equal(listed.length, 1);

  const inactive = await service.deactivateAnexo(ctx, quote.id, created.id);
  assert.equal(inactive.ativo, false);
  assert.equal(inactive.status, 'INATIVO');
  assert.equal((await service.listAnexos(ctx, quote.id)).length, 0);

  const events = await audit.listByEntity('OrcamentoAnexo', created.id);
