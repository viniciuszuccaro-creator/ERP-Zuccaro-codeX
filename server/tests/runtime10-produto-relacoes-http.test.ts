...
test('HTTP Produto persiste material, liga e norma sem vazar tenant', async () => {
  await withHttp(async (request) => {
    ...
    const cleared = await request(`${path}/${id}`, 'PATCH', { material: null, liga: null, norma_tecnica: null });
    assert.equal(cleared.status, 200);
    assert.equal(cleared.body.data.material, null);
    assert.equal((await request(`${path}/${id}`)).body.data.norma_tecnica, null);
    assert.equal((await request(`${path}/${id}`, 'GET', undefined, headers(GROUP_A, EMPRESA_A2))).status, 404);
    assert.equal((await request(`${path}/${id}`, 'PATCH', { material: '' })).status, 400);
    ...
    assert.equal(logs[2]?.beforeData?.material, 'Aco carbono');
    assert.equal(logs[2]?.afterData?.material, null);
