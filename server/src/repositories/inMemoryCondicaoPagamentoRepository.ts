...
export class InMemoryCondicaoPagamentoRepository implements CondicaoPagamentoRepository {
  ...
  private readonly nextCodigoByGroup = new Map<string, number>();
  ...
  private reserveCodigo(groupId:string){const highest=[...this.rows.values()].filter((row)=>row.group_id===groupId&&/^\d{6}$/.test(row.codigo)).reduce((value,row)=>Math.max(value,Number(row.codigo)),0);const next=Math.max(this.nextCodigoByGroup.get(groupId)??1,highest+1);this.nextCodigoByGroup.set(groupId,next+1);return String(next).padStart(6,'0');}
