-- Comercial 360 Onda 4 — anexos do Orçamento reutilizando DAM/StoragePort (aditiva).
-- Metadados no agregado Orçamento; binário só no storage. Sem módulo DAM paralelo.
...
CREATE TABLE IF NOT EXISTS orcamento_anexos (
  ...
  FOREIGN KEY (orcamento_id, group_id, empresa_id) REFERENCES orcamentos(id, group_id, empresa_id),
...
CREATE INDEX IF NOT EXISTS idx_orcamento_anexos_scope
  ON orcamento_anexos (group_id, empresa_id, orcamento_id, created_at DESC, id DESC);

ALTER TABLE orcamento_anexos ENABLE ROW LEVEL SECURITY;
ALTER TABLE orcamento_anexos FORCE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE orcamento_anexos FROM PUBLIC;
