...
http(){ curl --silent --show-error --output "$2" --write-out '%{http_code}' "$1"; }
assert_status(){ [ "$1" = "$2" ] || die "HTTP esperado=$2 recebido=$1"; }
wait_ready(){ local url="$1" n; for n in $(seq 1 30); do curl --fail --silent "$url/ready" >/dev/null 2>&1 && return 0; sleep 2; done; die "ready nao respondeu: $url"; }
cleanup(){ local status=$?; if [ "$CANARY_CREATED" = 1 ]; then log "cleanup fail-closed: parando somente $CANARY"; docker stop "$CANARY" >/dev/null 2>&1 || true; fi; exit "$status"; }
...
curl --fail --silent --show-error "$API_OFFICIAL/health" >/dev/null; curl --fail --silent --show-error "$API_OFFICIAL/ready" >/dev/null
node -e "const m=JSON.parse(process.argv[1]);if(m.runtime!=='ERP-RUNTIME-07B')process.exit(1)" "$(curl --fail --silent "$API_OFFICIAL/meta")" || die '3080 nao esta em ERP-RUNTIME-07B'
...
git -C "$ROOT" worktree add --detach "$WORKTREE" "$EXPECTED_MAIN"; [ "$(git -C "$WORKTREE" rev-parse HEAD)" = "$EXPECTED_MAIN" ] || die 'worktree incorreto'
...
docker run -d --name "$CANARY" --network "$NETWORK" --restart no --env-file "$ENV_FILE" -e PORT=3080 -p 127.0.0.1:3086:3080 "$IMAGE" >/dev/null; CANARY_CREATED=1
wait_ready "$API_CANARY"
node -e "const m=JSON.parse(process.argv[1]),c=m.condicaoPagamento;if(m.runtime!=='ERP-RUNTIME-08B'||!c||c.frontendHttp!==false||m.httpPilotEntities.includes('CondicaoPagamento'))process.exit(1)" "$(curl --fail --silent "$API_CANARY/meta")" || die 'metadata R08B invalida'
...
STATUS="$(curl --silent --show-error -o "$RUN_DIR/list.json" -w '%{http_code}' "${HEADERS[@]}" "$BASE")"; assert_status "$STATUS" 200
STATUS="$(http "$BASE" "$RUN_DIR/no-actor.json")"; assert_status "$STATUS" 403
STATUS="$(curl --silent --show-error -o "$RUN_DIR/cross-actor.json" -w '%{http_code}' -H "X-Group-Id: $GROUP_A" -H "X-Empresa-Id: $EMPRESA_A" -H "X-Actor-Id: $ACTOR_B" "$BASE")"; assert_status "$STATUS" 403
STATUS="$(curl --silent --show-error -o "$RUN_DIR/cross-company.json" -w '%{http_code}' -H "X-Group-Id: $GROUP_A" -H "X-Empresa-Id: $EMPRESA_B" -H "X-Actor-Id: $ACTOR_A" "$BASE")"; { [ "$STATUS" = 403 ] || [ "$STATUS" = 409 ]; } || die 'cross-company nao bloqueado'
PAYLOAD="{\"nome\":\"R08 canario $STAMP\",\"parcelas\":[{\"ordem\":1,\"dias\":30,\"percentual\":\"100.000000\"}]}"
STATUS="$(curl --silent --show-error -o "$RUN_DIR/create.json" -w '%{http_code}' -X POST "${HEADERS[@]}" -H 'Content-Type: application/json' --data "$PAYLOAD" "$BASE")"; assert_status "$STATUS" 201
...
STATUS="$(curl --silent --show-error -o "$RUN_DIR/get.json" -w '%{http_code}' "${HEADERS[@]}" "$BASE/$COND_ID")"; assert_status "$STATUS" 200
STATUS="$(curl --silent --show-error -o "$RUN_DIR/patch.json" -w '%{http_code}' -X PATCH "${HEADERS[@]}" -H 'Content-Type: application/json' --data '{"descricao":"canario controlado"}' "$BASE/$COND_ID")"; assert_status "$STATUS" 200
STATUS="$(curl --silent --show-error -o "$RUN_DIR/delete.json" -w '%{http_code}' -X DELETE "${HEADERS[@]}" "$BASE/$COND_ID")"; assert_status "$STATUS" 200
STATUS="$(curl --silent --show-error -o "$RUN_DIR/restore.json" -w '%{http_code}' -X POST "${HEADERS[@]}" "$BASE/$COND_ID/restore")"; assert_status "$STATUS" 200
...
docker restart "$CANARY" >/dev/null; wait_ready "$API_CANARY"; curl --fail --silent "$API_CANARY/health" >/dev/null; curl --fail --silent "$API_CANARY/ready" >/dev/null
node -e "const m=JSON.parse(process.argv[1]);if(m.runtime!=='ERP-RUNTIME-08B')process.exit(1)" "$(curl --fail --silent "$API_CANARY/meta")" || die 'metadata apos restart invalida'
...
node -e "const m=JSON.parse(process.argv[1]);if(m.runtime!=='ERP-RUNTIME-07B')process.exit(1)" "$(curl --fail --silent "$API_OFFICIAL/meta")" || die '3080 alterada'
