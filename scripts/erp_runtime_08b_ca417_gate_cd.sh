...
assert_runtime(){ python3 -c 'import json,sys; m=json.load(sys.stdin); assert m.get("runtime")==sys.argv[1]' "$1" <<< "$2"; }
assert_canary_meta(){ python3 -c 'import json,sys; m=json.load(sys.stdin); c=m.get("condicaoPagamento") or {}; assert m.get("runtime")=="ERP-RUNTIME-08B" and c.get("frontendHttp") is False and "CondicaoPagamento" not in m.get("httpPilotEntities",[])' <<< "$1"; }
extract_condicao(){ python3 -c 'import json,re,sys; d=json.load(sys.stdin).get("data") or {}; i=d.get("id"); c=d.get("codigo"); assert isinstance(i,str) and re.fullmatch(r"[0-9]{6}",str(c)); print(i,end="")' < "$1"; }
...
for command in docker curl git sha256sum python3; do need "$command"; done
...
assert_runtime 'ERP-RUNTIME-07B' "$(curl --fail --silent --max-time 10 "$API_OFFICIAL$META_PATH")" || die '3080 nao esta em ERP-RUNTIME-07B'
...
docker run --rm -v "$WORKTREE:/work" -w /work "$NODE_IMAGE" bash -lc 'npm ci && npm run audit:baseline && npm test && npm run lint && npm run typecheck && npm run build'
docker run --rm -e NODE_OPTIONS=--max-old-space-size=1536 -v "$WORKTREE/server:/work" -w /work "$NODE_IMAGE" bash -lc 'npm ci && node --import tsx --test --test-concurrency=1 tests/*.test.ts && npm run typecheck && npm run build'
...
export DATABASE_URL
set +e; docker run --rm --network "$NETWORK" -e DATABASE_URL -v "$WORKTREE/server:/app" -w /app "$NODE_IMAGE" bash -lc 'npm ci && npm run test:postgres' > "$RUN_DIR/postgres-e2e.log" 2>&1; POSTGRES_STATUS=$?; unset DATABASE_URL; set -e
...
assert_canary_meta "$(curl --fail --silent --max-time 10 "$API_CANARY$META_PATH")" || die 'metadata R08B invalida'
...
COND_ID="$(extract_condicao "$RUN_DIR/create.json")" || die 'codigo sequencial invalido'
...
assert_runtime 'ERP-RUNTIME-08B' "$(curl --fail --silent --max-time 10 "$API_CANARY$META_PATH")" || die 'metadata apos restart invalida'
...
assert_runtime 'ERP-RUNTIME-07B' "$(curl --fail --silent --max-time 10 "$API_OFFICIAL$META_PATH")" || die '3080 alterada'
