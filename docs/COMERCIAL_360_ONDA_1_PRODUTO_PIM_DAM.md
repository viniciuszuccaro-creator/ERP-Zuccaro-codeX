...
- Chaves de objetos seguem `groups/{groupId}/companies/{empresaId}/products/{produtoId}/{categoria}/{uuid}-{nome-sanitizado}`. O adapter valida escopo e categoria, extensao/MIME/tamanho, confere Content-Type, assinatura basica de PNG/JPEG/WebP/PDF/MP4, bytes e SHA-256 antes de devolver metadados. CAD/desenhos permanecem fora da allowlist ate validacao especifica e antivirus no gate DAM.
