...
test('gateway registra as quatro operações e mantém compatibilidade dos contratos anteriores', async () => {
  const source = await readFile(new URL('../base44/functions/_lib/security/siteCpaS2SPolicy/health.ts', import.meta.url), 'utf8');
  ...
  assert.match(source, /QUOTE_CREATE:/);
  assert.match(source, /NEGOTIATION:/);
  assert.match(source, /CUSTOMER_RESOLVE:/);
  assert.match(source, /CATALOG_READ:/);
  assert.match(source, /ORDER_CREATE:/);
});
