...
test('gate-d-f-precheck com candidatas 016-024 marca missing_for_gate_e', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gate-df-miss-'));
  const mig = path.join(tmp, 'migrations');
  fs.mkdirSync(mig);
  for (let i = 1; i <= 17; i += 1) {
    fs.writeFileSync(path.join(mig, `${String(i).padStart(3, '0')}_x.sql`), '-- synthetic\n');
  }
  const out = path.join(tmp, 'gate-c.txt');
  const lines = [];
  for (let i = 1; i <= 15; i += 1) {
  ...
  const run = spawnSync('bash', [
    ...
    '--candidate-migrations', mig,
  ], { encoding: 'utf8' });
  assert.equal(run.status, 0, run.stderr || run.stdout);
  assert.match(run.stdout, /missing_for_gate_e=016,017/);
  assert.match(run.stdout, /nao aplicar agora/i);
});
