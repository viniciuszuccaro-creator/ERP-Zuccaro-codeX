...
test('healthcheck S2S usa empresa padrao, audita e persiste o evento', async () => {
  ...
  assert.equal(result.body.data.timestamp, timestamp);
  assert.equal(result.body.data.capabilities.CUSTOMER_RESOLVE, 'blocked');
  assert.equal(result.body.data.capabilities.CATALOG_READ, 'blocked');
  assert.equal(result.body.data.capabilities.ORDER_CREATE, 'blocked');
  assert.equal(result.body.data.capabilities.QUOTE_CREATE, 'blocked');
  assert.equal(result.body.data.capabilities.NEGOTIATION, 'blocked');
  ...
  assert.equal(result.body.data.capabilities.CHAT, 'blocked');
  assert.equal(result.body.data.capabilities.PRODUCTION_RELEASE, 'blocked');
  assert.equal(result.body.data.capabilities.COMMERCIAL_COPILOT, 'blocked');
  assert.equal(result.body.data.dependencies.PAYMENT_PROVIDER, 'blocked');
  assert.equal(JSON.stringify(result.body).includes('service-token'), false);
